...
Game cards display each game's star rating from the seeded catalog, with the same rating component used on the game details page.
...
Repository coding standards are documented in the [Copilot instructions](.github/copilot-instructions.md) and the focused [Astro](.github/instructions/astro.instructions.md), [data-layer](.github/instructions/drizzle.instructions.md), [UI](.github/instructions/ui.instructions.md), [styling](.github/instructions/style.instructions.md), and [testing](.github/instructions/unit-tests.instructions.md) instruction files. In particular, comments should explain intent rather than restate code, exported data-layer functions require TSDoc/JSDoc for their purpose, parameters, and return values, and reusable Astro components must document their `Props` interface. ESLint enforces the documented TypeScript quote and semicolon conventions.
